...
(function(){
	...
	sy.ui.PagingBar.prototype = {
		...
		getLi: function(text, page, active){
			...
			if(page){
				if(!active){
					result = _.template('<li><a href="#" class="btn btn-default <%=cls %>" data-page="<%=page %>"><%=text %></a></li>',{
						text: text,
						page: page,
						cls: cls
					});
				}else{
					result = _.template('<li><a href="#" class="btn btn-primary <%=cls %> active" data-page="<%=page %>"><%=text %></a></li>',{
						text: text,
						page: page,
						cls: cls
					});
